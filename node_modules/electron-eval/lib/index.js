...
var Daemon = function (_EventEmitter) {
  ...
  function Daemon(opts) {
    ...
    var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(Daemon).call(this));
  ...
  _createClass(Daemon, [{
    ...
    value: function _startHeadless(cb) {
      ...
      headless(opts, function (err, child, display) {
        ...
        process.on('exit', function () {
          if (_this3.closing) return;
          process.kill(child.pid, 'SIGKILL');
        });
        _this3.xvfb = child;
        _this3.xDisplay = ':' + display;
        cb(null);
      });
    }
  }, {
    ...
    value: function _startElectron(opts, cb) {
      ...
      process.on('exit', function () {
        return _this4.child.kill();
      });

      if (!opts.nodeIPC) this._startIPC();

      this.child.once('message', function (data) {
        _this4.keepaliveInterval = setInterval(_this4.keepalive.bind(_this4), opts.timeout / 2);
        _this4.keepaliveInterval.unref();
        _this4.child.send(opts);
        _this4.child.once('message', function (data) {
          _this4.child.on('message', function (message) {
            return _this4.emit(message[0], message[1]);
          });
          _this4.ready = true;
          _this4.queue.forEach(function (item) {
            return _this4.eval.apply(_this4, _toConsumableArray(item));
          });
          _this4.queue = null;
          _this4.emit('ready');
          _this4.keepalive();
    ...
    value: function _startIPC() {
      ...
      stdout.on('data', function (data) {
        return _this5.child.emit('message', data);
      });
    }
  }]);
